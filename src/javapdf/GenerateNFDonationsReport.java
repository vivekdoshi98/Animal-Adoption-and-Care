...
public class GenerateNFDonationsReport {
	...
	public static void createNFDonationReport(String file,ArrayList<NonFinancialDonationBean> adopBean)
	{
	...
	private static void createAdoptionTable(Document document,ArrayList<NonFinancialDonationBean> adopList) throws DocumentException {
		...
		c1 = new PdfPCell(new Phrase("Donated Item"));
		c1.setHorizontalAlignment(Element.ALIGN_CENTER);
		table.addCell(c1);

		c1 = new PdfPCell(new Phrase("Donation Date"));
		c1.setHorizontalAlignment(Element.ALIGN_CENTER);
		table.addCell(c1);
		...
		for (int i = 0; i < adopList.size(); i++) {
			NonFinancialDonationBean adopBean = adopList.get(i);
			table.setWidthPercentage(100);
			table.getDefaultCell().setHorizontalAlignment(Element.ALIGN_CENTER);
			table.getDefaultCell().setVerticalAlignment(Element.ALIGN_MIDDLE);
			table.addCell(adopBean.getFirst_name());
			table.addCell(adopBean.getDonation_desc());
			table.addCell(adopBean.getDonation_date().substring(0, 11));
	...
	private static void addMetaData(Document document) {
		...
		document.addSubject("Non Financial Donations REPORT");
	...
	private static void addTitlePage(Document document)
		...
		creteEmptyLine(preface, 2);
		 ...
		 preface.add(new Paragraph("Non-Financial Donations REPORT", HELVETICA));
